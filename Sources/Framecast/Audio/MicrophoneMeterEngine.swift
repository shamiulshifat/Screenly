...
final class MicrophoneMeterEngine: NSObject {
    ...
    private let queue = DispatchQueue(label: "com.framecast.audio.meter", qos: .userInitiated)
    ...
    func start(device: AVCaptureDevice) throws {
        ...
        FramecastLogger.capture.info("Microphone meter started for \(device.localizedName)")
    }
