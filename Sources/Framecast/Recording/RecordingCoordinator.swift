...
final class RecordingCoordinator: NSObject, ObservableObject {
    ...
    func requestScreenPermission() async -> Bool {
        ...
        FramecastLogger.permissions.info("Requesting screen recording permission")
    ...
    func startRecording() async {
        ...
        do {
            try await recorder.start(
                ...
                excludingApplications: discovery.framecastApplications,
                excludingWindows: discovery.framecastWindows,
    ...
    func deleteRecording(at recordingURL: URL) {
        ...
        let bundleURL = recordingURL.deletingPathExtension().appendingPathExtension("framecast")
    ...
    private func restartMicrophoneMeter() {
        ...
        do {
            ...
            FramecastLogger.capture.error("Unable to start microphone meter: \(error.localizedDescription)")
    ...
    private func restartCameraPreview() {
        ...
        do {
            ...
            FramecastLogger.capture.error("Unable to start camera preview: \(error.localizedDescription)")
    ...
    private func persistMetadata(for outputURL: URL) {
        ...
        do {
            ...
            FramecastLogger.encoder.error("Failed writing metadata: \(error.localizedDescription)")
        }
    }
    ...
    private func updateSaveDirectoryDisplayPath() {
        if let custom = settingsStore.saveDirectoryURL?.path {
            ...
            saveDirectoryPath = (try? RecordingFileStore.defaultDirectory().path) ?? "~/Movies/Framecast"
        }
    }
