...
struct MainDashboardView: View {
    ...
    @State private var selectedQuickTab: QuickTab = .devices
    ...
    private var quickSettingsSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if showQuickSettings {
                Picker("Quick Tab", selection: $selectedQuickTab) {
                    ForEach(QuickTab.allCases) { tab in
                ...
                Group {
                    switch selectedQuickTab {
                    case .devices:
                        devicesQuickSettings
                    case .camera:
                        cameraQuickSettings
                    case .audio:
                        audioQuickSettings
    ...
    private var copyrightBlock: some View {
        VStack(alignment: .leading, spacing: 2) {
            Text("© MD SHAMIUL ISLAM SHIFAT")
            Text("BANGLADESH")
            Text("shifat.ruet@gmail.com")
        }
    ...
    private var devicesQuickSettings: some View {
        VStack(spacing: 8) {
    ...
    private var cameraQuickSettings: some View {
        VStack(spacing: 8) {
            quickField(title: "Camera") {
                HStack(spacing: 10) {
                    ...
                    Toggle("Mirror", isOn: $coordinator.cameraMirrorEnabled)
                        .toggleStyle(.switch)
            ...
            if coordinator.isCameraEnabled {
                ...
                quickField(title: "Preview (Uncropped)") {
                    ZStack {
                ...
                quickField(title: "Camera") {
                    Text("Enable camera to access effects and preview.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private var audioQuickSettings: some View {
        VStack(spacing: 8) {
            quickField(title: "Microphone") {
                HStack(spacing: 10) {
                    Toggle("Mute", isOn: microphoneMutedBinding)
                        .toggleStyle(.switch)
                    Toggle("System Audio", isOn: $coordinator.isSystemAudioEnabled)
                        .toggleStyle(.switch)
                }
            ...
            quickField(title: "Microphone Preview") {
                VStack(alignment: .leading, spacing: 8) {
                    HStack(spacing: 8) {
                    ...
                    HStack {
                        Text("RMS \(levelPercentText(coordinator.microphoneRMS))")
                        Spacer()
                        Text("Peak \(levelPercentText(coordinator.microphonePeak))")
                    }
                    .font(.caption.monospacedDigit())
                    .foregroundStyle(.white.opacity(0.72))

                    Text(coordinator.isMicrophoneMuted ? "Microphone is muted" : "Microphone is live")
                        .font(.caption)
                        .foregroundStyle(coordinator.isMicrophoneMuted ? .orange : .secondary)
                }
    ...
    private func levelPercentText(_ value: Float) -> String {
