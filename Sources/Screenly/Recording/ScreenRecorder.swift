...
final class ScreenRecorder: NSObject {
    ...
    private let sampleQueue = DispatchQueue(label: "com.screenly.capture.samples", qos: .userInitiated)
    private let controlQueue = DispatchQueue(label: "com.screenly.capture.control", qos: .userInitiated)
    ...
    func start(
        ...
        do {
            ...
            ScreenlyLogger.capture.error("Failed to add stream output: \(error.localizedDescription)")
        ...
        ScreenlyLogger.capture.info("Recording started to \(outputURL.path)")
    ...
    func updateCameraOverlay(_ overlay: CameraOverlayConfiguration) {
        ...
        do {
            ...
            ScreenlyLogger.capture.error("Unable to update camera overlay: \(error.localizedDescription)")
        }
    }
    ...
    func stop() async throws -> URL {
        ...
        do {
            ...
            ScreenlyLogger.capture.error("Failed stopping SCStream: \(error.localizedDescription)")
            throw RecordingError.streamStopFailed
        }
        ...
        let appendedFrameCount = controlQueue.sync {
            metricsState.appendedVideoFrames
        }

        if writer.status == .failed {
            let details = writer.error?.localizedDescription ?? "unknown"
            ScreenlyLogger.encoder.error("AssetWriter failed: \(details)")
            cleanupOutputArtifact(at: outputURL)
        ...
        if finalSize == 0 {
            ScreenlyLogger.encoder.error("Output is empty (frames=\(appendedFrameCount), bytes=\(finalSize))")
            self.cleanupOutputArtifact(at: outputURL)
            throw RecordingError.recordingWriteFailed
        }

        if appendedFrameCount == 0 {
            ScreenlyLogger.encoder.warning("Output finalized without observed frame appends (bytes=\(finalSize)); keeping file")
        }
        ...
        ScreenlyLogger.encoder.info("Recording finalized at \(finalizedURL.path)")
        return finalizedURL
    }
    ...
    private func personMask(for image: CIImage) -> CIImage? {
        ...
        do {
            ...
            ScreenlyLogger.capture.debug("Segmentation failed: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    private func mixAudioTracksIfNeeded(recordingURL: URL) async throws -> URL {
        ...
        guard exportSession.status == .completed else {
            ScreenlyLogger.encoder.error("Audio mix export failed: \(exportSession.error?.localizedDescription ?? "unknown")")
            return recordingURL
        }
        ...
        do {
            ...
            ScreenlyLogger.encoder.error("Failed replacing mixed output: \(error.localizedDescription)")
            return mixedURL
        }
...
extension ScreenRecorder: SCStreamOutput {
    func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of outputType: SCStreamOutputType) {
        ...
        switch outputType {
        case .screen:
            guard isCompleteScreenFrame(adjustedBuffer) else {
                controlQueue.sync {
    ...
    private func handleScreenSampleBuffer(_ sampleBuffer: CMSampleBuffer, writer: AVAssetWriter) {
        ...
        guard videoInput.isReadyForMoreMediaData else {
            ScreenlyLogger.encoder.debug("Dropping screen frame: writer input backpressure")
            controlQueue.sync {
                metricsState.droppedScreenFrames += 1
        ...
        if let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer),
           ...
           let adaptor = videoAdaptor {
            if !adaptor.append(compositedBuffer, withPresentationTime: presentationTime) {
                ...
                ScreenlyLogger.encoder.error("Failed to append composited video frame (writerStatus=\(writerStatus), writerError=\(writerDetails))")
                controlQueue.sync {
                    metricsState.droppedScreenFrames += 1
                }
            } else {
                controlQueue.sync {
                    metricsState.appendedVideoFrames += 1
        ...
        if !videoInput.append(sampleBuffer) {
            let writerStatus = writer.status.rawValue
            let writerDetails = writer.error?.localizedDescription ?? "none"
            ScreenlyLogger.encoder.error("Failed to append video sample (writerStatus=\(writerStatus), writerError=\(writerDetails))")
            controlQueue.sync {
                metricsState.droppedScreenFrames += 1
            }
        } else {
    ...
    private func isCompleteScreenFrame(_ sampleBuffer: CMSampleBuffer) -> Bool {
        guard let attachmentArray = CMSampleBufferGetSampleAttachmentsArray(sampleBuffer, createIfNecessary: false)
                as? [[AnyHashable: Any]],
    ...
    private func handleAudioSampleBuffer(_ sampleBuffer: CMSampleBuffer, input: AVAssetWriterInput?) {
        ...
        if !input.append(sampleBuffer) {
            ...
            let writerDetails = writer?.error?.localizedDescription ?? "none"
            ScreenlyLogger.encoder.error("Failed to append audio sample (writerStatus=\(writerStatus), writerError=\(writerDetails))")
        } else {
            let pts = CMSampleBufferGetPresentationTimeStamp(sampleBuffer)
            controlQueue.sync {
