...
final class GlobalShortcutManager {
    ...
    func registerDefaults() {
        ...
        guard status == noErr else {
            ScreenlyLogger.capture.error("Failed installing global hotkey handler: \(status)")
            return
        }
    ...
    private func register(action: Action, keyCode: UInt32, modifiers: UInt32) {
        ...
        guard status == noErr, let hotKeyRef else {
            ScreenlyLogger.capture.error("Failed registering hotkey for action \(action.rawValue): \(status)")
